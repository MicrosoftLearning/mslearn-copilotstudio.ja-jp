...
このラボでは、エージェントを作成し、指示を使用してそれに予想される動作を定義しました。 また、ナレッジ ソースとしてパブリック Web サイトを追加し、ナレッジ ソースが回答に役立つ可能性がある質問でエージェントをテストしました。 この後のラボでは、トピックや知識、ツールを使ってエージェントの動作をさらに制御する方法が紹介されます。
