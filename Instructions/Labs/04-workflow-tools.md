...
1. **[説明]** で、「`Post a message to Teams with the summary of the task analysis`」と入力します。
...
1. データを含む行と列 (A1:F5) を選択し、ツールバーで、**[挿入]** タブ、**[テーブル]** を選択し、**[テーブルにヘッダーがあります]** チェックボックスを選択して **[OK]** を選択します。
...
1. *[応答する値を入力する]* で、フィールドを選択し、[**式** (**fx**) オプションを選択します。 次の式を入力して、返された行を **テキスト** 出力の種類に一致するテキストに変換し、**[追加]** を選択します。
