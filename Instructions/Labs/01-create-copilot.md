...
1. Copilot Studio ホーム ページを表示します。これは次のような外観をしています。
    ...
    ホーム ページでは、エージェントの作成を開始したり、最近作業したエージェントを表示したりできます。 エージェントが定義されている Power Apps **環境**がページの上部に表示されます。

1. 左側のナビゲーション ウィンドウで、**[エージェント]** を選択して既存のエージェントを表示します。

    ![Copilot Studio エージェント ページのスクリーンショット。](media/copilot-studio-agents.png)

    > **注**:作成したエージェントに加えて、**Microsoft 365 Copilot** が表示される場合があります。これは、Copilot Studio を使用して拡張できます。

    完全に新しいエージェントを作成することも、エージェント テンプレートを出発点にすることもできます。 この演習では、構築するエージェントの機能を記述して、まったく新しいエージェントを作成します。
...
1. Copilot Studio によって作成されたエージェントをレビューします。
...
1. **[ファイルをアップロードする]** セクションで、**[選択して参照]** を使用して前にダウンロードした経費ポリシー ドキュメントをアップロードし、**[エージェントへの追加]** を選択します。
    ...
    > **注**:ファイルをアップロードした後、インデックスが作成されるまで待機する必要があります。これには 10 分以上かかる場合があります。
...
作業エージェントが作成されたので、ユーザーが使用できる状態であることをテストして確認できます。

1. ページの上部で **[テスト]** アイコンを選択して **[エージェントのテスト]** ペインを開きます。

1. **[エージェントのテスト]** ペインの**省略記号 [...]** メニューを選択し、**[トピック間の追跡]** を **[オン]** に切り替えます
