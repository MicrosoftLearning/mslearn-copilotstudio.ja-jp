...
このラボでは、以前のラボで収集された構造化された値 (プロパティの種類、プロパティ名、訪問日など) を使用して、Dataverse からデータを取得し、Dataverse で予約要求を作成します。
...
- Dataverse データを取得するためのエージェント フローを作成する
- Dataverse データを作成するためのエージェント フローを作成する
...
## 演習 1 - Dataverse からプロパティ データを取得するツールを作成する

この演習では、ユーザーが指定した条件に基づいてプロパティを取得するエージェント フローを作成します。

### タスク 1.1 - Get Property エージェント フローを作成する
...
1. エージェント フローの名前を `Get Property` に変更します

1. **[保存]** を選択します。

1. **[デザイナー]** タブを選択します。

### タスク 1.2 - Dataverse からデータを取得する
...
1. 認証を求められた場合は、**[接続名]** に「`Lab connection`」と入力して、**[認証の種類]** に **[OAuth]** を選択して、**[サインイン]** を選択し、自分のテナント資格情報を使用して **[アクセスを許可]** します。
...
1. **[エージェントに対して応答]** ウィンドウで **[設定]** タブを選択します。
...
1. Microsoft Copilot Studio で **[Real Estate Booking Service]** エージェントに戻ります。

1. **[ツール]** タブを選択します。

1. 先ほど作成した **[プロパティの取得]** エージェント フロー ツールを選択します。

   > [!NOTE]
   > このエージェント フローがツールの一覧に表示されない場合は、**[+ ツールの追加]** を選択し、**[ワークフロー]** タブを選択して、**[プロパティの取得]** エージェント フローを選択します。 **[追加と構成]** を選択します。

1. **[詳細]** セクションで、**[説明]** を `Get properties with the right number of bedrooms` に更新します。

1. **[保存]** を選びます。

1. **[ツール]** タブを選択し、作成した [プロパティの取得] フローを確認します。

1. **公開**を選択します。

### タスク 1.4 - Get Property ツールをトピックに追加する

1. **[Topics]** タブを選択します。

1. **[Book Showing]** トピックを選択します。

1. **[必要なベッドルームの数]** 質問ノードの下にある **+** アイコンを選択し、**[ツールの追加]** を選択し、**[ツール]** タブを選択してから、**[Get Property]** エージェント フローを選択します。

1. **[プロパティの取得]** アクション ウィンドウで、**[寝室 (文字列)]** の変数リストを選択し、**NumberofBedrooms** 変数を選択します。

1. **[どのプロパティを表示しますか?]** 質問ノードで**省略記号 ([...])** を選択し、**[削除]** を選択します。

1. **[ツール]** ノードの下にある **+** アイコンを選択し、**[メッセージの送信]** を選択します。
...
1. **[エージェントへの応答]** アクションを選択し、**[エージェントへの応答]** ウィンドウを開きます。
...
1. **Real Estate Booking Service** エージェントに戻ります。

1. **[ツール]** タブを選択し、両方のエージェント フローが一覧にあることを確認します。 一覧にない場合は、**[+ ツールの追加]**  >  **[ワークフロー]** の順に選択し、不足しているエージェント フローを選択します。 **[追加と構成]** を選択します。
...
1. 下部にある **[すべてのトピックを終了する]** ノードの上の **+** アイコンを選択し、**[ツールの追加]** を選択してから、**[予約要求の作成]** エージェント フローを選択します。
...
1. 予約要求の作成ツール用に作成したアクション ノードを選択します。

1. **[Copilot]** を選択します。

1. **[何をしますか?]** に、「`After this action runs, send a message that the Real Estate Booking has been scheduled and thank the user`」と入力します。

1. **[更新]** を選択します。

1. **[保存]** を選択します。

メッセージ ノードが作成されました。 必要に応じてメッセージを修正して、予約がスケジュールされていることをユーザーに知らせます。
...
予約要求が、エージェントに与えた情報に基づいて作成されているはずです。

### タスク 3.2 - 予約要求を確認する

1. 新しいタブで `https://make.powerapps.com` に移動します。
